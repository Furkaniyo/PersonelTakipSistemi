...
namespace PersonelTakipSistemi
{
    public partial class Form2 : Form
    {
        ...
        {
            if (textBox4.Text.Length != 8) errorProvider1.SetError(textBox4, "Kullanıcı Adı 8 Karakter Olmalı!");
        ...
        private void textBox5_TextChanged(object sender, EventArgs e)
        {
            ...
            string duzeltilmis_sifre = sifre.Replace('İ', 'I').Replace('ı', 'i').Replace('Ç', 'C').Replace('ç', 'c')
            ...
            else if (parola_skoru == 90 || parola_skoru == 100) parola_seviyesi = "Çok Güçlü";
        ...
        {
            if (textBox6.Text != textBox5.Text) errorProvider1.SetError(textBox6, "Parola Tekrarı Eşleşmiyor!");
            else errorProvider1.Clear();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                baglantim.Open();
                using (OleDbCommand selectsorgu = new OleDbCommand("select * from kullanicilar where tcno=@tcno", baglantim))
                {
                    selectsorgu.Parameters.AddWithValue("@tcno", tcnotextbox.Text);
                    using (OleDbDataReader kayitokuma = selectsorgu.ExecuteReader())
                    {
                        if (kayitokuma.Read()) kayitkontrol = true;
                    }
                }
                ...
                if (kayitkontrol == false)
                {
                    ...
                    if (tcnotextbox.Text.Length == 11 && tcnotextbox.Text != "" && textBox2.Text != "" && textBox2.Text.Length > 1 &&
                        textBox3.Text != "" && textBox3.Text.Length > 1 && textBox4.Text != "" && textBox4.Text.Length >= 8 &&
                        textBox5.Text != "" && textBox6.Text != "" && textBox5.Text == textBox6.Text && parola_skoru >= 70)
                    {
                        ...
                        try
                        {
                            using (OleDbCommand eklekomutu = new OleDbCommand("insert into kullanicilar (tcno, ad, soyad, yetki, kullaniciadi, parola) values (@tcno, @ad, @soyad, @yetki, @kuladi, @parola)", baglantim))
                            {
                                eklekomutu.Parameters.AddWithValue("@tcno", tcnotextbox.Text);
                                eklekomutu.Parameters.AddWithValue("@ad", textBox2.Text);
                                eklekomutu.Parameters.AddWithValue("@soyad", textBox3.Text);
                                eklekomutu.Parameters.AddWithValue("@yetki", yetki);
                                eklekomutu.Parameters.AddWithValue("@kuladi", textBox4.Text);
                                eklekomutu.Parameters.AddWithValue("@parola", textBox5.Text);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
                ...
                textBox6.Text != "" && textBox5.Text == textBox6.Text && parola_skoru >= 70)
            {
                ...
                try
                {
                    ...
                    {
                        baglantim.Open();
                        using (OleDbCommand guncellekomutu = new OleDbCommand("update kullanicilar set ad=@ad, soyad=@soyad, yetki=@yetki, kullaniciadi=@kuladi, parola=@parola where tcno=@tcno", baglantim))
                        {
                            guncellekomutu.Parameters.AddWithValue("@ad", textBox2.Text);
                            guncellekomutu.Parameters.AddWithValue("@soyad", textBox3.Text);
                            guncellekomutu.Parameters.AddWithValue("@yetki", yetki);
                            guncellekomutu.Parameters.AddWithValue("@kuladi", textBox4.Text);
                            guncellekomutu.Parameters.AddWithValue("@parola", textBox5.Text);
                            guncellekomutu.Parameters.AddWithValue("@tcno", tcnotextbox.Text);
            ...
            if (tcnotextbox.Text.Length == 11)
            {
                ...
                using (OleDbConnection baglantim = new OleDbConnection(baglantiCumlesi))
                {
                    ...
                    using (OleDbCommand selectsorgu = new OleDbCommand("select * from kullanicilar where tcno=@tcno", baglantim))
                    {
                        ...
                        using (OleDbDataReader kayitokuma = selectsorgu.ExecuteReader())
                        {
                            if (kayitokuma.Read()) kayit_arama_durumu = true;
                        }
                    }

                    if (kayit_arama_durumu == true)
                    {
                        using (OleDbCommand deletesorgu = new OleDbCommand("delete from kullanicilar where tcno=@tcno", baglantim))
                        {
                            deletesorgu.Parameters.AddWithValue("@tcno", tcnotextbox.Text);
                    ...
                    using (OleDbCommand selectsorgu = new OleDbCommand("select * from personeller where tcno=@tcno", baglantim))
                    {
                        ...
                        using (OleDbDataReader kayitokuma = selectsorgu.ExecuteReader())
                        {
                            if (kayitokuma.Read())
                            {
                                ...
                                {
                                    if (girisTarihi < DateTime.Today) dateTimePicker2.MinDate = girisTarihi;
                                    else dateTimePicker2.MinDate = DateTime.Today;
                                }
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            if (maskedTextBox4.MaskCompleted == false || (maskedTextBox4.Text != "" && int.Parse(maskedTextBox4.Text) < 1000)) label21.ForeColor = Color.Red;
            else label21.ForeColor = Color.FromArgb(226, 232, 240);
            ...
            {
                if (radioButton3.Checked == true) cinsiyet = "Bay"; else if (radioButton4.Checked == true) cinsiyet = "Bayan";
                ...
                try
                {
                    ...
                    {
                        baglantim.Open();
                        using (OleDbCommand guncellekomutu = new OleDbCommand("update personeller set ad=@ad, soyad=@soyad, cinsiyet=@cinsiyet, mezuniyet=@mezuniyet, dogumtarihi=@dogumtarihi, gorevi=@gorevi, gorevyeri=@gorevyeri, maasi=@maasi, giristarihi=@giris where tcno=@tcno", baglantim))
                        {
                            guncellekomutu.Parameters.AddWithValue("@ad", maskedTextBox2.Text);
                            guncellekomutu.Parameters.AddWithValue("@soyad", maskedTextBox3.Text);
                            guncellekomutu.Parameters.AddWithValue("@cinsiyet", cinsiyet);
                            guncellekomutu.Parameters.AddWithValue("@mezuniyet", comboBox1.Text);
                            guncellekomutu.Parameters.AddWithValue("@dogumtarihi", dateTimePicker1.Text);
                            guncellekomutu.Parameters.AddWithValue("@gorevi", comboBox2.Text);
                            guncellekomutu.Parameters.AddWithValue("@gorevyeri", comboBox3.Text);
                            guncellekomutu.Parameters.AddWithValue("@maasi", maskedTextBox4.Text);
                            guncellekomutu.Parameters.AddWithValue("@giris", dateTimePicker2.Text);
                            guncellekomutu.Parameters.AddWithValue("@tcno", maskedTextBox1.Text);
                            guncellekomutu.ExecuteNonQuery();
                        }
                    }
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection baglantim = new OleDbConnection(baglantiCumlesi))
            {
                ...
                if (kayitkontrol == false)
                {
                    ...
                    if (comboBox2.Text == "") label19.ForeColor = Color.Red; else label19.ForeColor = Color.FromArgb(226, 232, 240);
                    if (comboBox3.Text == "") label20.ForeColor = Color.Red; else label20.ForeColor = Color.FromArgb(226, 232, 240);
                    if (dateTimePicker2.Value.Date < DateTime.Today) label23.ForeColor = Color.Red; else label23.ForeColor = Color.FromArgb(226, 232, 240);
                    if (maskedTextBox4.Text == "" || (maskedTextBox4.Text != "" && int.Parse(maskedTextBox4.Text) < 1000)) label21.ForeColor = Color.Red; else label21.ForeColor = Color.FromArgb(226, 232, 240);
                    ...
                    if (pictureBox2.Image != null && maskedTextBox1.MaskCompleted != false && maskedTextBox2.MaskCompleted != false && maskedTextBox3.MaskCompleted != false && comboBox1.Text != "" && comboBox2.Text != "" && comboBox3.Text != "" && label21.ForeColor != Color.Red)
                    {
                        ...
                        try
                        {
                            using (OleDbCommand eklekomutu = new OleDbCommand("insert into personeller (tcno, ad, soyad, cinsiyet, mezuniyet, dogumtarihi, gorevi, gorevyeri, maasi, giristarihi) values(@tcno, @ad, @soyad, @cinsiyet, @mezuniyet, @dogumtarihi, @gorevi, @gorevyeri, @maasi, @giris)", baglantim))
                            {
                                eklekomutu.Parameters.AddWithValue("@tcno", maskedTextBox1.Text);
                                eklekomutu.Parameters.AddWithValue("@ad", maskedTextBox2.Text);
                                eklekomutu.Parameters.AddWithValue("@soyad", maskedTextBox3.Text);
                                eklekomutu.Parameters.AddWithValue("@cinsiyet", cinsiyet);
                                eklekomutu.Parameters.AddWithValue("@mezuniyet", comboBox1.Text);
                                eklekomutu.Parameters.AddWithValue("@dogumtarihi", dateTimePicker1.Text);
                                eklekomutu.Parameters.AddWithValue("@gorevi", comboBox2.Text);
                                eklekomutu.Parameters.AddWithValue("@gorevyeri", comboBox3.Text);
                                eklekomutu.Parameters.AddWithValue("@maasi", maskedTextBox4.Text);
                                eklekomutu.Parameters.AddWithValue("@giris", dateTimePicker2.Text);
                                eklekomutu.ExecuteNonQuery();
                            }
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            if (maskedTextBox1.MaskCompleted == true)
            {
                ...
                {
                    baglantim.Open();
                    using (OleDbCommand arama_sorgusu = new OleDbCommand("select * from personeller where tcno=@tcno", baglantim))
                    {
                        arama_sorgusu.Parameters.AddWithValue("@tcno", maskedTextBox1.Text);
                        using (OleDbDataReader kayitokuma = arama_sorgusu.ExecuteReader())
                        {
                            if (kayitokuma.Read()) kayit_arama_durumu = true;
                        }
                    }
                    ...
                    if (kayit_arama_durumu == true)
                    {
                        using (OleDbCommand deletesorgu = new OleDbCommand("delete from personeller where tcno=@tcno", baglantim))
                        {
                            deletesorgu.Parameters.AddWithValue("@tcno", maskedTextBox1.Text);
                            deletesorgu.ExecuteNonQuery();
                        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                if (yetki == "Kullanıcı") radioButton1.Checked = true;
                else if (yetki == "Yönetici") radioButton2.Checked = true;
        ...
        private void dataGridView2_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                if (cinsiyet == "Bay") radioButton3.Checked = true;
                else if (cinsiyet == "Bayan") radioButton4.Checked = true;
